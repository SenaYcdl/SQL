...
-- SORU6: Her markanin ismini, calisan sayisini ve o markaya ait calisanlarin maksimum ve 
...
as max_maas, (select min(maas) from calisanlar where marka_isim=isyeri) as min_maas from markalar

-- SORU7: Her markanin id’sini, ismini ve toplam kaç şehirde bulunduğunu 
--listeleyen bir SORGU yaziniz.
select marka_id,marka_isim, (select count(sehir) from calisanlar where marka_isim=isyeri)as
sehir_sayisi from markalar
